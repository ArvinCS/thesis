...
from clustered_flat_tree_adapter import ClusteredFlatTreeAdapter
...
class MultiDayTrafficSimulator:
    ...
    def __init__(self, documents, properties_by_province, seed=42):
        ...
        self.daily_patterns = {
            ...
            'Friday': {'base_multiplier': 1.1, 'peak_hours': [9, 10, 11, 14, 16]},
...
class MultiDayVerificationSuite:
    ...
    def _setup_contracts(self):
        ...
        try:
            ...
            # Traditional huffman contract
            traditional_huffman_artifact_path = '../../artifacts/contracts/TraditionalHuffmanMerkleVerifier.sol/TraditionalHuffmanMerkleVerifier.json'
            with open(traditional_huffman_artifact_path, 'r') as f:
                traditional_huffman_artifact = json.load(f)

            traditional_huffman_address = traditional_huffman_artifact['networks']['31337']['address']
            self.traditional_huffman_contract = self.web3.eth.contract(
                address=traditional_huffman_address,
                abi=traditional_huffman_artifact['abi']
            )
            
            # Single proof contract
            single_proof_artifact_path = '../../artifacts/contracts/TraditionalSingleproofMerkleVerifier.sol/TraditionalSingleproofMerkleVerifier.json'
            ...
            self.traditional_singleproof_contract = self.web3.eth.contract(
            ...
            self.traditional_singleproof_contract = None
            self.clustered_flat_contract = None
            self.clustered_flat_with_merkle_contract = None
    
    def run_comprehensive_multi_day_test(self, document_count=3000, num_days=7, base_events_per_day=300, force_onchain_verification=False, sparse_verification=False, verification_sampling_rate=1.0, selected_approaches=None, reports_dir=None):
        """
        ...
        try:
            ...
            tree_systems = self._build_all_tree_systems(documents, traffic_data, selected_approaches)
            ...
            final_report = self._generate_comprehensive_report(
                ...
                optimization_analysis, overall_start_time, reports_dir
            )
    ...
    def _build_all_tree_systems(self, documents, traffic_data, selected_approaches=None):
        """Build tree systems for comparison based on selected approaches."""
        # Store traffic data for simulation but don't pre-feed to builders
        # Builders start with empty traffic logs and learn from actual verifications
        self.simulation_traffic_data = traffic_data
        ...
        if 'hierarchical' in selected_approaches:
            ...
            # Start with empty traffic logs - will learn from actual verifications
            from compressed_traffic_logs import CompressedTrafficLogs
            empty_traffic_logs = CompressedTrafficLogs()
            jurisdiction_manager = JurisdictionTreeManager(documents, empty_traffic_logs)
        ...
        if 'traditional_huffman' in selected_approaches:
            ...
            from compressed_traffic_logs import CompressedTrafficLogs
            empty_traffic_logs = CompressedTrafficLogs()
            traditional_huffman_builder = TraditionalMultiproofWithHuffmanBuilder(documents, empty_traffic_logs)
            huffman_root = traditional_huffman_builder.build()
            huffman_build_time = time.time() - huffman_start
        
        if 'clustered_flat' in selected_approaches or 'clustered_flat_with_merkle' in selected_approaches:
            print("Building Clustered Flat and Basic Tree System...")
            clustered_flat_start = time.time()
            clustered_flat_adapter = ClusteredFlatTreeAdapter()
            # Start with empty traffic logs - will learn from actual verifications
            clustered_flat_system = clustered_flat_adapter.build_tree_system(documents, None)
            clustered_flat_build_time = time.time() - clustered_flat_start
            
            # Initialize daily learning for clustered flat
            if hasattr(clustered_flat_adapter, 'builder') and hasattr(clustered_flat_adapter.builder, 'reset_daily_learning'):
                clustered_flat_adapter.builder.reset_daily_learning()
        ...
        if 'clustered_flat' in selected_approaches:
            tree_systems['clustered_flat'] = {
                'adapter': clustered_flat_adapter,
                'system': clustered_flat_system,
                'root': clustered_flat_system['root'],
                'build_time': clustered_flat_build_time,
                'type': 'Clustered Flat Tree'
            }
        ...
        if 'clustered_flat_with_merkle' in selected_approaches:
            tree_systems['clustered_flat_with_merkle'] = {
                ...
                'root': clustered_flat_system['root'],
                'build_time': clustered_flat_build_time,
                'type': 'Clustered Flat Tree (MerkleVerifier)'
            }
        ...
        if 'clustered_flat' in selected_approaches or 'clustered_flat_with_merkle' in selected_approaches:
            print(f"  Clustered Flat Tree: {clustered_flat_build_time:.3f}s")
        if 'clustered_flat_with_merkle' in selected_approaches:
            print(f"  Clustered Flat Tree (MerkleVerifier): {clustered_flat_build_time:.3f}s")
    ...
    def _test_single_day(self, tree_systems, day_traffic, day_idx, day_stats):
        ...
        # For comprehensive gas analysis, test all events when doing on-chain verification
        # For other tests, sample events to avoid overwhelming the system
        if hasattr(self, 'force_onchain_verification') and self.force_onchain_verification:
            test_events = day_traffic  # Test ALL events for accurate gas analysis
            print(f"  📊 Testing ALL {len(test_events)} events for comprehensive gas analysis")
        else:
            test_events = random.sample(day_traffic, min(50, len(day_traffic)))
            print(f"  📊 Testing {len(test_events)} sampled events (out of {len(day_traffic)} total)")
    ...
    def _test_single_verification_event(self, approach_name, system, event, day_idx, event_idx, end_of_day=False):
        ...
        if approach_name == 'hierarchical':
            ...
            estimated_gas = None
            actual_gas_used = None
            if self.traditional_multiproof_contract:
                try:
                    ...
                    if self.force_onchain_verification:
                        ...
                        # First estimate gas
                        estimated_gas = self.traditional_multiproof_contract.functions.verifyBatch(
            ...
            estimated_gas = None
            if self.traditional_singleproof_contract:
                try:
                    total_gas = 0
                    for proof_data in individual_proofs:
                        ...
                        gas_estimate = self.traditional_singleproof_contract.functions.verifySingle(
            ...
            if self.traditional_singleproof_contract:
                try:
                    ...
                    for proof_data in individual_proofs:
                        ...
                        if self.force_onchain_verification:
                            # Perform actual on-chain verification
                            print(f"      Executing on-chain traditional singleproof verification...")
                            
                            # First estimate gas
                            total_estimated_gas += self.traditional_singleproof_contract.functions.verifySingle(
                                proof_bytes, leaf_bytes
                            ).estimate_gas()
                            
                            # Execute actual transaction
                            tx_hash = self.traditional_singleproof_contract.functions.verifySingle(
                                proof_bytes, leaf_bytes
                            ).transact()
                            
                            # Wait for transaction receipt and get actual gas used
                            receipt = self.web3.eth.wait_for_transaction_receipt(tx_hash)
                            total_actual_used_gas += receipt.gasUsed
                            
                            print(f"      ✅ On-chain verification successful - Gas used: {actual_gas_used}")
                            
                        else:
                            # Just estimate gas
                            estimated_gas = self.traditional_singleproof_contract.functions.verifySingle(
                                proof_bytes, flags, leaves_bytes
                            ).estimate_gas()
                            total_estimated_gas += estimated_gas
                    estimated_gas = total_estimated_gas
                    actual_gas_used = total_actual_used_gas
                except Exception as e:
                    estimated_gas = f"Error: {e}"
            ...
            # Traditional multiproof with Huffman optimization
            all_doc_hashes = []
            for prop_id in event:
                for doc in system['builder'].all_documents:
                    if doc.full_id == prop_id:
            ...
            # Generate multiproof with Huffman optimization
            proof, flags = system['builder'].generate_proof_for_documents(all_doc_hashes)
            proof_size = len(proof) * 32 + len(flags) + len(all_doc_hashes) * 32
            
            # Use traditional contract for gas estimation (same interface)
            estimated_gas = None
            actual_gas_used = None
            if self.traditional_huffman_contract:
                try:
                    # Perform actual on-chain verification
                    print(f"      Executing on-chain traditional singleproof with huffman verification...")

                    proof_bytes = [bytes.fromhex(p) for p in proof]
                    leaves_sorted = sorted(all_doc_hashes)
                    leaves_bytes = [bytes.fromhex(l) for l in leaves_sorted]
                    
                    estimated_gas = self.traditional_huffman_contract.functions.verifyBatch(
                        proof_bytes, flags, leaves_bytes
                    ).estimate_gas()
                    
                    tx_hash = self.traditional_huffman_contract.functions.verifyBatch(
                        proof_bytes, flags, leaves_bytes
                    ).transact()
            ...
            # Clustered Flat Tree approach
            # Prepare document data for verification
            documents_to_verify = []
            builder = system['system']['builder']
            
            # Create a mapping from full_id to hash for quick lookup
            doc_hash_map = {}
            for doc in builder.all_documents:
                doc_hash_map[doc.full_id] = doc.hash_hex
            
            # Find document hashes for this event
            for prop_id in event:
                if prop_id in doc_hash_map:
                    documents_to_verify.append(doc_hash_map[prop_id])
            
            # Deduplicate while preserving order
            documents_to_verify = list(dict.fromkeys(documents_to_verify))
            
            # If no documents found through hash map, try to find from ordered leaves
            if not documents_to_verify:
                # Alternative: try to find documents in the builder's ordered leaves
                # This should work for the clustered flat tree structure
                for prop_id in event:
                    for doc in builder.all_documents:
                        if (hasattr(doc, 'full_id') and doc.full_id == prop_id) or \
                           (hasattr(doc, 'property_id') and f"{doc.province}.{doc.property_id}" == prop_id):
                            if doc.hash_hex in builder.ordered_leaves_hex:
            ...
            if not documents_to_verify:
                # Debug information
                available_props = [doc.full_id for doc in builder.all_documents[:5]]
                return {
                    'approach': approach_name,
                    'day': day_idx,
                    'event': event_idx,
                    'error': f'No documents found for event. Event props: {event[:3]}, Available: {available_props}'
                }
            
            # Generate verification data using adapter
            adapter = system['adapter']
            verification_data = adapter.generate_verification_data(system['system'], documents_to_verify)
            
            # Calculate proof size
            proof_size = (len(verification_data['proof']) * 32 + 
                         len(verification_data['proof_flags']) + 
                         len(verification_data['leaves']) * 32)
            ...
            estimated_gas = None
            actual_gas_used = None
            if self.clustered_flat_contract:
                try:
                    contract_data = adapter.prepare_contract_call_data(verification_data)
                    
                    if self.force_onchain_verification:
                        # Perform actual on-chain verification
                        print(f"      Executing on-chain clustered flat verification...")
                        
                        # First estimate gas
                        estimated_gas = self.clustered_flat_contract.functions.verifyClusteredFlatMultiproof(
                            contract_data['proof'],
                            contract_data['proofFlags'],
                            contract_data['leaves'],
                            contract_data['root']
                        ).estimate_gas()
                        
                        # Execute actual transaction
                        tx_hash = self.clustered_flat_contract.functions.verifyClusteredFlatMultiproof(
                            contract_data['proof'],
                            contract_data['proofFlags'],
                            contract_data['leaves'],
                            contract_data['root']
            ...
            # Clustered Flat Tree approach using MerkleVerifier for comparison
            # This allows benchmarking the same tree structure with different contracts
            documents_to_verify = []
            builder = system['system']['builder']
            ...
            if not documents_to_verify:
                return {
                    ...
                    'day': day_idx,
                    'event': event_idx,
                    'error': f'No documents found for event'
                }
            
            # Generate verification data using adapter (same as clustered_flat)
            adapter = system['adapter']
            verification_data = adapter.generate_verification_data(system['system'], documents_to_verify)
            ...
            # Local verification using the same logic
            builder = system['system']['builder']
            is_valid, message = builder.verify_multiproof_locally(
                verification_data['proof'],
                verification_data['proof_flags'],
            ...
            # Use MerkleVerifier instead of ClusteredFlatVerifier
            estimated_gas = None
            actual_gas_used = None
            if self.clustered_flat_basic_contract:
                try:
                    # Convert data for MerkleVerifier.verifyBatch()
                    proof_bytes = [bytes.fromhex(p.replace('0x', '')) for p in verification_data['proof']]
                    leaves_bytes = [bytes.fromhex(l.replace('0x', '')) for l in verification_data['leaves']]
                    
                    # First update the merkle root to match clustered flat root
                    if self.force_onchain_verification:
                        print(f"      Executing on-chain clustered flat verification via MerkleVerifier...")
                        
                        # Estimate gas first
                        estimated_gas = self.clustered_flat_basic_contract.functions.verifyBatch(
                            proof_bytes, verification_data['proof_flags'], leaves_bytes
                        ).estimate_gas()
                        
                        # Execute actual transaction
                        tx_hash = self.clustered_flat_basic_contract.functions.verifyBatch(
                            proof_bytes, verification_data['proof_flags'], leaves_bytes
                        ).transact()
                        ...
                        # Just estimate gas (need to set root first)
                        current_root = verification_data['root']
                        if not current_root.startswith('0x'):
                            current_root = '0x' + current_root
                        
                        # For estimation, we can't actually update root, so this might fail
                        # But we'll try anyway for comparison
                        estimated_gas = self.clustered_flat_basic_contract.functions.verifyBatch(
                            proof_bytes, verification_data['proof_flags'], leaves_bytes
                        ).estimate_gas()
    ...
    def _generate_comprehensive_report(self, documents, traffic_data, tree_systems, 
                                     verification_results, optimization_analysis, start_time, reports_dir=None):
        ...
        if reports_dir:
            ...
            report_path = os.path.join(reports_dir, "multi_day", report_filename)
            os.makedirs(os.path.dirname(report_path), exist_ok=True)
            with open(report_path, 'w') as f:
                import json
                json.dump(report, f, indent=2)
            report_file = report_path
            print(f"💾 Saved: multi_day/{report_filename}")
        else:
            # Use old report organizer
    ...
    def run_cross_province_focused_test(self, documents, properties_by_province, traffic_logs, force_onchain_verification=False):
        ...
        tree_systems = self._build_all_tree_systems(documents, {"traffic_logs": traffic_logs})
    ...
    def _test_cross_province_verification_event(self, approach_name, system, event, event_idx, is_4plus_province, end_of_day=False):
        ...
        if approach_name == 'hierarchical':
            ...
            # Generate individual proofs for each document using the correct method name
            total_proof_size = 0
            estimated_gas = 0
            actual_gas_used = 0
            for doc_hash in all_doc_hashes:
                proof = system['builder'].generate_single_proof_for_document(doc_hash)
                total_proof_size += len(proof) * 32
            ...
            # Traditional multiproof with Huffman approach (same as traditional_multiproof but with Huffman optimizations)
            all_doc_hashes = []
            for prop_id in event:
                for doc in system['builder'].all_documents:
                    if doc.full_id == prop_id:
            ...
            # Generate Huffman-optimized multiproof
            proof, flags = system['builder'].generate_proof_for_documents(all_doc_hashes)
            proof_size = len(proof) * 32 + len(flags) + len(all_doc_hashes) * 32
            ...
            if self.traditional_huffman_contract:
                try:
                    if self.force_onchain_verification:
                        estimated_gas = self.traditional_huffman_contract.functions.verifyClusteredFlatMultiproof(
                            proof,
                            flags,
                            all_doc_hashes
                        ).estimate_gas()

                        tx_hash = self.traditional_huffman_contract.functions.verifyClusteredFlatMultiproof(
                            proof,
                            flags,
                            all_doc_hashes
                        ).transact()
            ...
            # Clustered Flat Tree approach using MerkleVerifier (for benchmarking comparison)
            documents_to_verify = []
            builder = system['system']['builder']
            ...
            # Generate verification data using the same adapter as clustered_flat
            adapter = system['adapter']
            verification_data = adapter.generate_verification_data(system['system'], documents_to_verify)
            ...
            # Local verification using the same logic as clustered_flat
            is_valid, message = builder.verify_multiproof_locally(
                verification_data['proof'],
                verification_data['proof_flags'],
                verification_data['leaves']
            ...
            if self.clustered_flat_basic_contract:
                try:
                    ...
                    if self.force_onchain_verification:
                        # Estimate and execute verification
                        estimated_gas = self.clustered_flat_basic_contract.functions.verifyBatch(
                            proof_bytes, verification_data['proof_flags'], leaves_bytes
                        ).estimate_gas()
                        ...
                        # For estimation only, we can't update root, so this might not work perfectly
                        estimated_gas = self.clustered_flat_basic_contract.functions.verifyBatch(
                            proof_bytes, verification_data['proof_flags'], leaves_bytes
                        ).estimate_gas()
    ...
    def _analyze_cross_province_performance(self, verification_results):
        """Analyze performance specifically for cross-province scenarios across all approaches."""
        from collections import defaultdict
        
        # Separate 4+ province events from others
        results_4plus = [r for r in verification_results if r is not None and r.get('is_4plus_province', False) and 'error' not in r]
        results_other = [r for r in verification_results if r is not None and not r.get('is_4plus_province', False) and 'error' not in r]
        
        # Get all available approaches
        all_approaches = list(set(r['approach'] for r in verification_results if r is not None and 'error' not in r))
        
        # Get number of approaches tested (for calculating events tested)
        num_approaches = len(all_approaches) if all_approaches else 1
        
        analysis = {
            'total_4plus_events_tested': len(results_4plus) // num_approaches if num_approaches > 0 else 0,
            'total_other_events_tested': len(results_other) // num_approaches if num_approaches > 0 else 0,
            ...
            'best_approach_per_event': [],
            'overall_best_approach': None,
            'gas_cost_rankings': {}
        }
        
        # Analyze all events (both 4+ province and others)
        all_results = results_4plus + results_other
        
        # Group results by event
        events_by_id = defaultdict(list)
        for result in all_results:
            if 'estimated_gas' in result and isinstance(result['estimated_gas'], int):
                events_by_id[result['event']].append(result)
        
        # Analyze each event to find best performing approach
        approach_wins = defaultdict(int)
        approach_gas_savings = defaultdict(list)
        best_cases_by_approach = defaultdict(list)
        
        for event_id, event_results in events_by_id.items():
            if len(event_results) < 2:  # Need at least 2 approaches to compare
                continue
                
            # Find the best (lowest gas) approach for this event
            best_result = min(event_results, key=lambda x: x['estimated_gas'])
            worst_result = max(event_results, key=lambda x: x['estimated_gas'])
            
            # Calculate savings for each approach compared to worst
            for result in event_results:
                if result['estimated_gas'] < worst_result['estimated_gas']:
                    savings_percent = ((worst_result['estimated_gas'] - result['estimated_gas']) / worst_result['estimated_gas']) * 100
                    approach_gas_savings[result['approach']].append(savings_percent)
            
            # Record win for best approach
            approach_wins[best_result['approach']] += 1
            
            # Store detailed comparison
            event_analysis = {
                'event': event_id,
                'best_approach': best_result['approach'],
                'best_gas': best_result['estimated_gas'],
                'worst_gas': worst_result['estimated_gas'],
                'gas_savings_percent': ((worst_result['estimated_gas'] - best_result['estimated_gas']) / worst_result['estimated_gas']) * 100,
                'provinces_count': best_result['provinces_count'],
                'properties_count': best_result['properties_count'],
                'provinces_list': best_result.get('provinces_list', []),
                'is_4plus_province': best_result.get('is_4plus_province', False),
                'all_results': {r['approach']: r['estimated_gas'] for r in event_results}
            }
            analysis['best_approach_per_event'].append(event_analysis)
            
            # Track best cases for each approach
            for result in event_results:
                if result['estimated_gas'] == best_result['estimated_gas']:
                    best_cases_by_approach[result['approach']].append(event_analysis)
        
        # Calculate overall performance metrics for each approach
        total_events = len(events_by_id)
        for approach in all_approaches:
            wins = approach_wins.get(approach, 0)
            win_rate = (wins / total_events * 100) if total_events > 0 else 0
            avg_savings = sum(approach_gas_savings[approach]) / len(approach_gas_savings[approach]) if approach_gas_savings[approach] else 0
            
            analysis['approach_performance'][approach] = {
                'wins': wins,
                'win_rate': win_rate,
                'avg_gas_savings_when_winning': avg_savings,
                'best_cases_count': len(best_cases_by_approach[approach])
            }
        
        # Determine overall best approach
        if approach_wins:
            analysis['overall_best_approach'] = max(approach_wins.items(), key=lambda x: x[1])[0]
        
        # Legacy hierarchical vs traditional analysis for backward compatibility
        hierarchical_results = [r for r in all_results if r['approach'] == 'hierarchical']
        traditional_results = [r for r in all_results if r['approach'] == 'traditional_multiproof']
        
        hierarchical_wins = []
        traditional_wins = []
        
        for h_result in hierarchical_results:
            # Find corresponding traditional result
            t_result = next((r for r in traditional_results if r['event'] == h_result['event']), None)
        ...
        # Create gas cost rankings
        if events_by_id:
            # Calculate average gas cost for each approach across all events
            approach_avg_gas = {}
            for approach in all_approaches:
                approach_results = [r for r in all_results if r['approach'] == approach]
                if approach_results:
                    avg_gas = sum(r['estimated_gas'] for r in approach_results) / len(approach_results)
                    approach_avg_gas[approach] = avg_gas
            
            # Sort approaches by average gas cost (ascending - lower is better)
            sorted_approaches = sorted(approach_avg_gas.items(), key=lambda x: x[1])
            analysis['gas_cost_rankings'] = {
                'by_average_gas': sorted_approaches,
                'best_efficiency': sorted_approaches[0][0] if sorted_approaches else None,
                'worst_efficiency': sorted_approaches[-1][0] if sorted_approaches else None
            }
    ...
    def _add_learning_event(self, approach_name, system, event, end_of_day=False):
        ...
        try:
            ...
            if config.mode == LearningMode.DISABLED:
                print(f"      📚 Learning is disabled for {approach_name}")
                return
            
            if config.verbose_logging:
                print(f"      📚 Adding learning event for {approach_name} in mode {config.mode}")
            
            should_rebuild = False
            current_root = None
            if approach_name == 'hierarchical' and 'manager' in system:
                # For hierarchical approach, add to each affected province tree
                manager = system['manager']
                if hasattr(manager, 'province_builders'):
                    provinces = set(prop_id.split('.')[0] for prop_id in event if '.' in prop_id)
                    if config.verbose_logging:
                        print(f"      Affected provinces: {provinces}")
                        print(f"      Available province builders: {list(manager.province_builders.keys())}")
            ...
            print(f"      🔄 {approach_name} learnt. Rebuild: {should_rebuild}. Root: {current_root[:16] if current_root else 'None'}. Thanks!")
            if should_rebuild:
                # Update root of their smart contract
                if self.web3 and self.web3.is_connected() and current_root:
                    # Convert hex string to bytes32 format
                    if current_root.startswith('0x'):
                        root_bytes = bytes.fromhex(current_root[2:])
                    else:
                        root_bytes = bytes.fromhex(current_root)

                    if approach_name == 'hierarchical' and self.hierarchical_contract:
                        tx_hash = self.hierarchical_contract.functions.updateJurisdictionRoot(root_bytes).transact()
                        self.web3.eth.wait_for_transaction_receipt(tx_hash)
                    elif approach_name == 'traditional_huffman' and self.traditional_huffman_contract:
                        tx_hash = self.traditional_huffman_contract.functions.updateRoot(root_bytes).transact()
                        self.web3.eth.wait_for_transaction_receipt(tx_hash)
                    elif approach_name == 'clustered_flat' and self.clustered_flat_contract:
                        tx_hash = self.clustered_flat_contract.functions.updateRoot(root_bytes).transact()
                        self.web3.eth.wait_for_transaction_receipt(tx_hash)
                    elif approach_name == 'clustered_flat_with_merkle' and self.clustered_flat_basic_contract:
                        tx_hash = self.clustered_flat_basic_contract.functions.updateRoot(root_bytes).transact()
                        self.web3.eth.wait_for_transaction_receipt(tx_hash)
                if config.verbose_logging:
                    print(f"      🔄 {approach_name} tree rebuilt and contract root updated")

        except Exception as e:
            # Safe error handling - don't rely on config being available
            print(f"      ⚠️ Error adding learning event for {approach_name}: {e}")
