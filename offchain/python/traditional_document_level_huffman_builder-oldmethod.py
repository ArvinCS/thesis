...
class TraditionalDocumentLevelHuffmanBuilder:
    ...
    def _get_document_pair_frequencies(self):
        ...
        # Group documents by property for co-access analysis
        property_docs = {}
        ...
        # Generate pairs within each property (documents accessed together)
            ...
            for i, doc1 in enumerate(prop_docs):
                for j, doc2 in enumerate(prop_docs[i+1:], i+1):
                    ...
                    if pair_key not in doc_pair_freq:
                        doc_pair_freq[pair_key] = 0
                    
                    # Weight by document importance and co-access probability
                    doc1_type = doc1.doc_id.split('_')[1] if '_' in doc1.doc_id else doc1.doc_id
                    doc2_type = doc2.doc_id.split('_')[1] if '_' in doc2.doc_id else doc2.doc_id
                    
                    doc1_freq = self.transactional_pattern.document_importance_map.get(doc1_type, 1)
                    doc2_freq = self.transactional_pattern.document_importance_map.get(doc2_type, 1)
                    
                    # Co-access frequency is typically lower than individual access
                    # Use minimum frequency as base and apply co-access factor
                    pair_freq = min(doc1_freq, doc2_freq) * 0.7
    ...
    def _apply_pairs_first_huffman(self, documents, doc_frequencies, pair_frequencies):
            ...
            if freq >= min_pair_freq_threshold:  # Alpha threshold check
                if doc1 not in merged_docs and doc2 not in merged_docs:
                    if doc1 in doc_nodes and doc2 in doc_nodes:
                        ...
                        # Update document-to-node mapping for both documents
                        old_node1 = doc_nodes[doc1]
                        old_node2 = doc_nodes[doc2]
        ...
        # Step 3: Collect unique nodes (avoid duplicates from pair merging)
