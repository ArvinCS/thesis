#!/usr/bin/env python3
"""
Clustered Province with Document-Level Huffman Tree Builder

This module combines province clustering with true pairs-first Huffman optimization:
1. Primary Sorting (Clustering): Group leaves by jurisdiction (province)
2. Secondary Sorting (Property Grouping): Within each province, group documents by property
3. Tertiary Sorting (Pairs-First Document Huffman): Within each property group, apply true
   pairs-first Huffman with alpha threshold for unbalanced but efficient trees

Features:
- Province-based clustering for jurisdictional efficiency
- Property grouping within provinces
- Pairs-first document-level Huffman with alpha threshold (shallow depth first)
- Unbalanced tree optimization (follows research paper methodology)
- Iterative proof generation compatible with IterativeMultiProofVerifier
...
class PropertyDocumentGroup:
    ...
    def optimize_document_order(self, document_pair_frequencies, document_frequencies):
        ...
        if len(self.documents) > 20:  # Threshold for computational efficiency
...
class ClusteredProvinceWithDocumentHuffmanBuilder:
    """
    Builds a single flat Merkle tree with three-level optimization:
    1. Primary: Cluster by province (jurisdiction)
    2. Secondary: Group by property within province
    3. Tertiary: Apply document-level Huffman within property groups
    """
                ...
                for prop_id, group in province_groups.items():
                    if len(group.documents) >= 2:
                        doc_pair_freq = self.transactional_pattern.get_document_pair_frequencies(
                            group.documents, num_simulated_queries=5000  # Reduced for efficiency
                        )
                ...
                for group in province_groups.values():
                    if len(group.documents) >= 2:
                        ...
                        pattern_pair_freq = self.transactional_pattern.get_document_pair_frequencies(
                            group.documents, num_simulated_queries=1000  # Reduced for efficiency
                        )
    ...
    def build(self):
        """
        Build the clustered province with document Huffman tree with three-level optimization:
        1. Primary: Group by province
        2. Secondary: Group by property within province  
        3. Tertiary: Apply document-level Huffman within property groups
        """
        print("Building Clustered Province with Document-Level Huffman Tree...")
        ...
        sorted_provinces = sorted(self.province_property_groups.keys())
        
        self.ordered_leaves_hex = []
        
        for province in sorted_provinces:
            property_groups = self.province_property_groups[province]
            
            print(f"  Optimizing {province}: {len(property_groups)} properties")
            
            # Secondary sorting: Order properties within province by frequency
            sorted_properties = sorted(
                property_groups.keys(),
                key=lambda prop_id: property_freq.get(f"{province}.{prop_id}", 0),
                reverse=True
            )
            
            for property_id in sorted_properties:
                property_group = property_groups[property_id]
                
                print(f"    Optimizing documents in {province}.{property_id}: {len(property_group.documents)} documents")
                
                # Only apply expensive document-level Huffman for properties with multiple documents
                if len(property_group.documents) > 1:
                    # Tertiary sorting: Apply document-level Huffman within property group
                    property_group.optimize_document_order(document_pair_frequencies, document_frequencies)
                else:
                    # For single document properties, no optimization needed
                    property_group.optimized_document_order = property_group.documents
                
                # Add optimized document hashes to final ordering
                self.ordered_leaves_hex.extend(property_group.get_document_hashes_hex())
