...
class ClusteredProvinceWithDocumentHuffmanBuilder:
    ...
    def _build_property_huffman_tree(self, property_groups, property_freq, pair_freq):
        ...
        if pair_freq:
            ...
            min_pair_freq_threshold = max_pair_freq * alpha_threshold
            print(f"    Property-level alpha threshold: {alpha_threshold} -> Min pair frequency: {min_pair_freq_threshold:.1f}")
            ...
            for (prop1_id, prop2_id), freq in sorted_pairs:
                if freq >= min_pair_freq_threshold:
                    ...
                    if node1 and node2 and node1 not in merged_pairs and node2 not in merged_pairs:
                        ...
                        merged_node.all_documents = node1.get_all_documents() + node2.get_all_documents()
        ...
        while len(heap) > 1:
            ...
            merged.left = left
            merged.right = right
            merged.all_documents = left.get_all_documents() + right.get_all_documents()
